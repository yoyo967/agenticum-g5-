  ...
  return (
    <div className="w-80 bg-void border-l border-steel flex flex-col h-full overflow-hidden select-none font-mono relative z-20">
      <div className="p-8 border-b border-steel bg-obsidian/95 relative overflow-hidden group">
        ...
        <h3 className="text-[11px] font-black text-chrome uppercase tracking-[0.6em] relative z-10">Artifact_Vault</h3>
        <div className="mt-2 text-[7px] text-tungsten font-black uppercase tracking-widest opacity-40">Storage_Allocated: {(assets.length * 1.2).toFixed(2)}MB</div>
      </div>

      <div className="flex-1 overflow-y-auto scrollbar-hide space-y-px bg-steel/10 custom-scrollbar">
        {assets.length === 0 ? (
          ...
          assets.map((asset) => (
            <div 
              ...
              className="group cursor-pointer bg-obsidian/40 hover:bg-steel/5 border-b border-steel/10 p-6 relative transition-all duration-300"
            >
              <div className="flex justify-between items-center mb-3">
                ...
                {asset.label.includes('REFINED') && (
                  <div className="flex items-center gap-1.5">
                    <div className="w-1.5 h-1.5 bg-amethyst rounded-full animate-pulse shadow-[0_0_8px_#aa00ff]" />
                    <span className="text-[7px] text-amethyst font-black uppercase">Optimized</span>
                  </div>
                )}
              </div>
              <div className="aspect-video bg-void border border-steel/20 relative flex items-center justify-center overflow-hidden">
                {asset.type === 'IMAGE' ? (
                  <img src={asset.url} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-all duration-700" />
                ) : (
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-6 h-6 border border-steel/40 rotate-45" />
              ...
              <div className="mt-4">
                ...
                <span className="text-[7px] text-tungsten opacity-30 uppercase font-black block mt-1.5">{asset.timestamp} // VERIFIED</span>
              </div>
            </div>
          ))
        )}
      ...
      <div className="p-8 border-t border-steel bg-obsidian/95">
        <button 
          ...
          className="w-full py-5 bg-neon/5 border border-neon/30 text-[10px] font-black text-neon hover:bg-neon hover:text-void transition-all uppercase tracking-[0.6em] disabled:opacity-20 active:scale-95"
        >
          Forge_Full_Archive
        </button>
      </div>
